...
def recommend_by_category(category, top_n=6):
    ...
    if subset.empty:
        return pd.DataFrame([{"name": "No foods found", "calories": 0, "protein": 0,
                              "fat": 0, "carbohydrate": 0, "fiber": 0, "sugars": 0}])
    return subset.sample(n=min(top_n, len(subset)))[['name', 'calories', 'protein', 'fat',
                                                     'carbohydrate', 'fiber', 'sugars', 'category']]
...
def model_rank_for_category(target_category, top_n=6):
    ...
    try:
        ...
        return df_sorted.head(top_n)[['name', 'calories', 'protein', 'fat',
                                      'carbohydrate', 'fiber', 'sugars', 'category', 'score']]
    except Exception:
        return None
